...
public class FAWEWorldNativeAccess implements WorldNativeAccess<Chunk, IBlockData, BlockPosition> {
    private static final int UPDATE = 1;
    private static final int NOTIFY = 2;

    private final FAWESpigotV16R3 adapter;
    private final WeakReference<World> world;
    private SideEffectSet sideEffectSet;
    private final AtomicInteger lastTick;
    private final Set<CachedChange> cachedChanges = new ConcurrentSet<>();

    public FAWEWorldNativeAccess(FAWESpigotV16R3 adapter, WeakReference<World> world) {
        ...
        RunnableVal<Object> r = new RunnableVal<Object>() {
            ...
            public void run(Object value) {
                ...
                for (IntPair chunk : toSend) {
                    FAWEBukkitAdapter.sendChunk(getWorld().getWorld().getHandle(), chunk.x, chunk.z, 0, false);
                }
            }
        };
        if (Fawe.isMainThread()) {
